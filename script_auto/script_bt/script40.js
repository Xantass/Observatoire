...
.then(response => {
    ...
    axios.get("https://api.insee.fr/series/BDM/V1/data/SERIES_BDM/001710973", {
        headers: {
            ...
            if (err) {
                ...
                if (obsElements && obsElements.length > 0) {
                    connexion.connect(async (erreur) => {
                        ...
                        connexion.query("TRUNCATE TABLE indice_bt40", (erreur, resultats) => {
                            if (erreur) {
                        ...
                        const requeteInsertion = 'INSERT INTO indice_bt40 (BT_DATE, BT_INDICE, BT_PARUTION) VALUES (?, ?, ?)';
