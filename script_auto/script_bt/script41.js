...
.then(response => {
    ...
    axios.get("https://api.insee.fr/series/BDM/V1/data/SERIES_BDM/001710974", {
        headers: {
            ...
            if (err) {
                ...
                if (obsElements && obsElements.length > 0) {
                    connexion.connect(async (erreur) => {
                        ...
                        connexion.query("TRUNCATE TABLE indice_bt41", (erreur, resultats) => {
                            if (erreur) {
                        ...
                        const requeteInsertion = 'INSERT INTO indice_bt41 (BT_DATE, BT_INDICE, BT_PARUTION) VALUES (?, ?, ?)';
